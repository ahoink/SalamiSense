...
void HW_Reset_state(uint8_t Usb_Port)
{
  ...
  HAL_Delay(15);  /*time to be detected by the source */
...
void ALARM_MANAGEMENT(uint8_t Usb_Port)   
{
  ...
  if ( HAL_GPIO_ReadPin(GPIOB,EXTI5_PD_I2C_ALERT_Pin)== GPIO_PIN_RESET)
  {
    Address = ALERT_STATUS_1; 
...
void Print_SNK_PDO(uint8_t Usb_Port)  
{
  ...
  // Does not work if instantiated here
#ifdef PRINTF
...
#endif 
        sprintf(buf, " - Fixed PDO%u=(%4.2fV, %4.2fA, = %uW)\r\n",i+1, PDO_V, PDO_I, PDO_P );
        HAL_UART_Transmit(&huart1, (uint8_t *)buf, sizeof(buf), HAL_MAX_DELAY);
...
#endif 
        sprintf(buf, " - Variable PDO%u=(%4.2fV, %4.2fV, = %4.2fA)\r\n",i+1, PDO_V_Min, PDO_V_Max, PDO_I );
        HAL_UART_Transmit(&huart1, (uint8_t *)buf, sizeof(buf), HAL_MAX_DELAY);
      }
      break;
...
#endif 
        sprintf(buf, " -Battery PDO%u=(%4.2fV, %4.2fV, = %uW)\r\n",i+1, PDO_V_Min, PDO_V_Max, PDO_P );
        HAL_UART_Transmit(&huart1, (uint8_t *)buf, sizeof(buf), HAL_MAX_DELAY);
...
void Print_PDO_FROM_SRC(uint8_t Usb_Port)  
{
  ...
  for (i=0; i< PDO_FROM_SRC_Num[Usb_Port]; i++)
  {
    ...
    sprintf(" - PDO_FROM_SRC(%u)=(%4.2fV, %4.2fA, = %uW)\r\n",i+1, PDO_V, PDO_I, PDO_P);
    HAL_UART_Transmit(&huart1, (uint8_t *)buf, sizeof(buf), HAL_MAX_DELAY);
